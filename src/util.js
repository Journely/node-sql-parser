...
function literalToSQL(literal) {
  ...
  result.push(str);
  if (suffix && suffix.collate)
    result.push(commonTypeValue(suffix.collate).join(" "));
  str = result.join(" ");
...
function arrayStructTypeToSQL(expr) {
  ...
  const isNotArrayOrStruct =
    dataTypeUpper !== "ARRAY" && dataTypeUpper !== "STRUCT";
...
function autoIncreatementToSQL(autoIncreatement) {
  if (!autoIncreatement || typeof autoIncreatement === "string")
    return toUpper(autoIncreatement);
  const { keyword, seed, increment, parentheses } = autoIncreatement;
  let result = toUpper(keyword);
  if (parentheses)
    result += `(${literalToSQL(seed)}, ${literalToSQL(increment)})`;
  return result;
}
