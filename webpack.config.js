...
const getCopyFile = (database) => {
  return [
    {
      source: `${outputPath}/${database}.js`,
      destination: `${outputPath}/build/${database}.js`,
    },
    {
      source: `${outputPath}/${database}.js.map`,
      destination: `${outputPath}/build/${database}.js.map`,
    },
    {
      source: `${outputPath}/${database}.d.ts`,
      destination: `${outputPath}/build/${database}.d.ts`,
    },
  ];
};
...
const getSrcFile = () =>
  fs
    .readdirSync(srcPath)
    .filter((name) => name !== "parser.all.js" || name !== "parser.single.js")
    .map((name) => ({
      source: `${outputPath}/${name}`,
      destination: `${outputPath}/lib/${name}`,
    }));
        ...
        new FileManagerPlugin({
          events: {
            onEnd: {
              ...
              copy: [
                {
                  source: `${outputPath}/*.umd.d.ts`,
                  destination: `${outputPath}/umd`,
                },
                {
                  source: `${outputPath}/*.umd.js`,
                  destination: `${outputPath}/umd`,
                },
                {
                  source: `${outputPath}/*.umd.js.map`,
                  destination: `${outputPath}/umd`,
                },
              ],
              move: [...getDbFile(), ...getSrcFile()],
              delete: [
                `${outputPath}/*.umd.d.ts`,
                `${outputPath}/*.umd.js`,
...
const getOutput = (target) => ({
  ...
  devtoolModuleFilenameTemplate: (info) =>
    path.resolve(__dirname, info.resourcePath),
});
...
if (isProd) {
  ...
  for (const target of ["web", "node"]) {
    config.push(
      ...
      ...fs
        .readdirSync(path.join(__dirname, "pegjs"))
        .map((x) => /^(.+)\.pegjs$/.exec(x))
        .filter((x) => !!x)
        .map((x) => x[1])
        .map((parser) =>
